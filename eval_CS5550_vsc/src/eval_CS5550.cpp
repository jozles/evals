...
#define SS  A2      // spi CS
#define INT 5 // nc on dets       // cs5550 INT pin
...
void setup() {
  ...
  a=inch((const char*)"input g when shunt shortened ; d default ");
  if(a=='g'){calibrateOffset();}
  if(a=='d'){
    cs5550RegWrite(SS, AIN1OFF, cal1Off);
    cs5550RegWrite(SS, AIN2OFF, cal2Off);
    }
  a=inch((const char*)"input g when max current settled ; d default ");
...
void calibrateOffset()
{
  ...
  delay(2000);
  cs5550Command(SS,CAL1_);

  cs5550Command(SS,CAL2O);
  delay(2000);
  cs5550Command(SS,CAL2_);
